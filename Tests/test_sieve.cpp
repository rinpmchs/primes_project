...
TEST(SieveTest, TimeComplexityTest) {
    Detail::Sieve sieve;
    Profiler::start();
    std::vector<SmallInteger> primes = sieve.generate_primes(1000000);
    Profiler::finish();
    ...
    sieve_test_results << "SieveTest, TimeComplexityTest," << 1000000 << "," << 1 << "," << duration << "\n";
}

TEST(SieveTest, EdgeCaseTest) {
    Detail::Sieve sieve;
    Profiler::start();
