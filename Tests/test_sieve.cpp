...
TEST(Sieve, GeneratePrimes) {
    ...
    EXPECT_EQ(generate_primes(30), expected_30);
    assert(!expected_79.empty() && "expected_79 is empty");
    EXPECT_EQ(generate_primes(79), expected_79);
    assert(!expected_1000000.empty() && "expected_1000000 is empty");
    EXPECT_EQ(generate_primes(1000000), expected_1000000);
}

TEST(Sieve, TimeComplexityTest) {
    Profiler::start();
    std::vector<SmallInteger> primes = generate_primes(1000000);
    Profiler::finish();
...
TEST(Sieve, EdgeCaseTest) {
    std::vector<SmallInteger> primes = generate_primes(1);
